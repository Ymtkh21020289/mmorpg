...
function update() {
    ...
    if (this.keys.attack.isDown && !this.isTyping ) {
        ...
        if (weapon.weapontype === 'direct'){
            ...
            this.enemies.getChildren().forEach((enemy) => {
                ...
                if (distance < weapon.radius) {
                    ...
                    if (Math.abs(angleDiff) < weapon.range * Math.PI / 180) {
                        ...
                        // ヒット確定！
                        this.socket.emit('attackEnemy', { enemyId: enemy.id, damage: weapon_damage, ratio: slotItem.stats.ratio || 0 });
            ...
            const damage = Math.floor((this.totalAtk / 4) + (this.maxMp /2) + weapon_damage);
...
function createMenuUI(scene) {
    ...
    // ステータス画面の「戻る」ボタン
    const btnBack = scene.add.rectangle(0, 150, 200, 40, 0x333333).setScrollFactor(0).setInteractive({ useHandCursor: true });
    ...
    // はい
    const btnDieYes = scene.add.rectangle(0, 40, 200, 40, 0xaa0000).setScrollFactor(0).setInteractive({ useHandCursor: true });
    ...
    // いいえ
    const btnDieNo = scene.add.rectangle(0, 100, 200, 40, 0x333333).setScrollFactor(0).setInteractive({ useHandCursor: true });
